...
function FurnitureImageUploadScreen({ onBack, onUploaded }: { onBack: () => void, onUploaded: () => void }) {
  ...
  const handleUpload = async () => {
    ...
    if (successCount > 0) {
      toast({ title: "Thành công", description: `Đã upload ${successCount} ảnh nội thất!` });
    }
    if (failCount > 0) {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Upload Mẫu Nội thất mới</h1>
      <div className="flex gap-2 items-end">
        ...
        <Button onClick={handleUpload} disabled={!files || files.length === 0 || isUploading}>
          {isUploading ? "Đang upload..." : "Upload mẫu nội thất"}
        </Button>
      ...
      {files && files.length > 0 && (
        <div className="mt-2 text-sm text-muted-foreground">
          Đã chọn {files.length} mẫu nội thất: {Array.from(files).map(f => f.name).join(", ")}
        </div>
      )}
    </div>
  );
...
export default function FurnitureAdminPage() {
  ...
  const handleEditCaption = async (id: string, newCaption: string) => {
    ...
    await updateFurnitureImageCaption(id, newCaption);
    fetchImages();
  };

  // Nếu đang ở màn hình upload ảnh thì render màn hình upload
  if (showUpload) {
    return <FurnitureImageUploadScreen onBack={() => setShowUpload(false)} onUploaded={fetchImages} />;
  }
